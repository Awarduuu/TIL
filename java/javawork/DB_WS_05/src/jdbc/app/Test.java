...
public class Test {
	static AccountDao accountDAOImpl = AccountDaoImpl.getInstance();
...
//			selectUserAccountCnt();
	...
	static void insert() {
		...
		if(accountDAOImpl.insert(accountDto) == 0) {
	...
	static void update() {
		...
		if(accountDAOImpl.update(accountDto) == 0) {
	...
	static void selectAll() {
		...
		List<AccountDto> accountList = accountDAOImpl.selectAll();
	...
	static void selectOne() {
		...
		int accountSeq = 80;
		AccountDto accountDto = accountDAOImpl.selectOne(accountSeq);
		
		// 이 곳에 AccountDao 를 이용해서 account table 을 1건 조회하여 AccountDto 객체를 만들고 객체의 내용을 출력하는 코드를 작성한다.
		// 위 accountSeq 를 이용해서 1건  조회한다.
		if(accountDto == null) {
			System.out.println("******* " + accountSeq + " 데이터 조회 실패 ********");
		}else {
			System.out.println("******* " + accountSeq + " 데이터 조회 ********");
			System.out.println(accountDto);
		}
	}
	...
	static void delete() {
		...
		if(accountDAOImpl.delete(accountSeq)==0) {
			System.out.println("******* " + accountSeq + " 데이터 실패 ********");

		}else {
			System.out.println("******* " + accountSeq + " 데이터 삭제 ********");
		}
	}
	...
	static void selectUsersAccountByName() {
		...
		List<AccountDto> accountList = accountDAOImpl.selectUsersAccountByName(searchName);
		...
		System.out.println("******* " + searchName + " 계좌 조회 ********");
		for(AccountDto account : accountList) {
			System.out.println(account);
		}
	}
	...
	static void selectUserAccountCnt() {
		...
		List<UserAccountCntDto> userAccountCntList = accountDAOImpl.selectUserAccountCnt();
		...
		System.out.println("******* 계좌 개수 조회 ********");
		for(UserAccountCntDto cnt : userAccountCntList) {
